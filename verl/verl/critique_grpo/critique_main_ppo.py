...
ray.init(
    _system_config={
        ...
        "gcs_rpc_server_reconnect_timeout_s": 6000
    },
    # _temp_dir="/mnt/workspace/xyzhang/temp",
...
class RewardManager():
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        # correctness = reward_tensor.bool()
        if return_dict:
            return {
...
@hydra.main(config_path="config", config_name="critique_ppo_trainer", version_base=None)
def main(config):
    run_ppo(config)
...
class TaskRunner:
    def run(self, config):
        ...
        # print("config.actor_rollout_ref.actor.strategy: ", config.actor_rollout_ref.actor.strategy)

        # define worker classes
        if config.actor_rollout_ref.actor.strategy in ["fsdp", "fsdp2"]:
            assert config.critic.strategy in ["fsdp", "fsdp2"]
            from verl.single_controller.ray import RayWorkerGroup
            # print("actor rollout strategy: ", config.actor_rollout_ref.actor.strategy) fsdp
            # from verl.workers.fsdp_workers import ActorRolloutRefWorker, AsyncActorRolloutRefWorker, CriticWorker
            from .critique_fsdp_workers import ActorRolloutRefWorker, AsyncActorRolloutRefWorker, CriticWorker

            print("config.actor_rollout_ref.rollout.mode: ", config.actor_rollout_ref.rollout.mode)
        ...
        from .critique_ray_trainer import ResourcePoolManager, Role
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor):
    ...
    from .rl_dataset_with_critique import RLHFDataset
